...
class MainActivity : AppCompatActivity() {
  ...
  private fun getMoviesFragment(): Fragment {
    when (navItemIndex) {
      TAG_NOW -> {
        return movieFragment.newinstance(TAG_NOW)
      }
      TAG_POPULAR -> {
        return movieFragment.newinstance(TAG_POPULAR)
      }
      TAG_TOP_RATED -> {
        return movieFragment.newinstance(TAG_TOP_RATED)
      }
      TAG_UPCOMING -> {
        return movieFragment.newinstance(TAG_UPCOMING)
      }
      else -> return movieFragment.newinstance(TAG_NOW)
    }
  }
